...
def parse_inkml(inkml_file_abs_path, return_label=False):
    if inkml_file_abs_path.endswith('.inkml'):
        ...
        #convert integers to float before 
        traces_ids, traces_raw = zip(*[[trace_tag.get('id'), [[float(axis_coord) for axis_coord in coord.strip(' ').split(' ')] 
                                                                                    for coord in (trace_tag.text).replace('\n', '').split(',')]] 
                                                                                        for trace_tag in root.findall(doc_namespace + 'trace')])
        t_i = 0
        in_inches = False
        while t_i < len(traces_raw) and not in_inches:
            coord_i = 0
            while coord_i < len(traces_raw[t_i]) and not in_inches:
                in_inches = all([not axis_coord.is_integer() for axis_coord in traces_raw[t_i][coord_i]])
                coord_i += 1
            t_i += 1

        if in_inches:
            traces_raw = [[[round(axis_coord * 1000) for axis_coord in coord] for coord in trace]for trace in traces_raw]
        else:
            traces_raw = [[[int(axis_coord) for axis_coord in coord] for coord in trace]for trace in traces_raw]

        traces_all_list = [{"id": t_id, "coords": trace} for t_id, trace in zip(traces_ids, traces_raw)]
        ...
        if return_label:
            return traces_all, root.find(doc_namespace+"annotation[@type='truth']").text
        return traces_all
    else:
...
if __name__ == '__main__':
    ...
    ''' 2 usages :
    convertInkmlToImg.py file.inkml (dim) (padding)
    convertInkmlToImg.py folder (dim) (padding)
